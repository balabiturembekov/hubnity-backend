# 🎯 Реализация детекции простоя (Idle Detection)
...
2. Обновить настройки:
   ...
   {
     "idleDetectionEnabled": true,
     "idleThreshold": 300  // 5 минут
   }
   ```

## 🎯 Логика работы

1. **Клиент отправляет heartbeat** каждые 30-60 секунд
2. **Сервер обновляет** `UserActivity.lastHeartbeat`
3. **Cron job каждую минуту:**
   - Находит все компании с включенной детекцией простоя
   - Для каждой компании находит пользователей с активными time entries
   - Проверяет простоя каждого пользователя
   - Если пользователь в простое (последний heartbeat > `idleThreshold`):
     - Ставит time entry на паузу
     - Отправляет WebSocket событие
     - Обновляет статус `isIdle` в `UserActivity`

## 📝 Примечания

- По умолчанию детекция простоя **включена** для всех компаний
- По умолчанию порог простоя: **300 секунд (5 минут)**
- Минимальный порог: **60 секунд (1 минута)**
- Максимальный порог: **3600 секунд (1 час)**
- Cron job работает **каждую минуту**, поэтому максимальная задержка паузы = 1 минута + время обработки
...
```bash
# В Docker контейнере или локально
npx prisma db push
```
