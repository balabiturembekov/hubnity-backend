...
export class TimeEntriesController {
  ...
  @ApiOperation({ 
    ...
    description: 'Создает новую запись времени. Сотрудники могут создавать записи только для себя. Для сотрудников проект обязателен.',
  })
  @ApiResponse({ status: 201, description: 'Запись времени успешно создана' })
  @ApiResponse({ status: 400, description: 'Неверные данные запроса или у пользователя уже есть активная запись' })
  @ApiResponse({ status: 403, description: 'Недостаточно прав доступа' })
  @ApiResponse({ status: 404, description: 'Пользователь или проект не найден' })
  ...
  @ApiOperation({ 
    ...
    description: 'Возвращает список записей времени. Администраторы могут фильтровать по userId и projectId. Сотрудники видят только свои записи.',
  })
  @ApiQuery({ name: 'userId', required: false, description: 'ID пользователя для фильтрации (только для админов)', type: String })
  @ApiQuery({ name: 'projectId', required: false, description: 'ID проекта для фильтрации', type: String })
  ...
  @ApiOperation({ 
    ...
    description: 'Возвращает список активных (RUNNING или PAUSED) записей времени',
  })
  @ApiQuery({ name: 'userId', required: false, description: 'ID пользователя для фильтрации (только для админов)', type: String })
  ...
  @ApiResponse({ status: 200, description: 'Список записей времени пользователя' })
  ...
  @ApiQuery({ name: 'userId', required: false, description: 'ID пользователя для фильтрации (только для админов)', type: String })
  @ApiQuery({ name: 'limit', required: false, description: 'Лимит записей (максимум 1000, по умолчанию 100)', type: Number, example: 100 })
  ...
  @ApiOperation({ 
    ...
    description: 'Останавливает активную запись времени и вычисляет финальную длительность',
  })
  @ApiParam({ name: 'id', description: 'ID записи времени', type: String })
  ...
  @ApiResponse({ status: 400, description: 'Запись времени не может быть приостановлена' })
  @ApiResponse({ status: 403, description: 'Недостаточно прав доступа' })
  @ApiResponse({ status: 404, description: 'Запись времени не найдена' })
  ...
  @ApiResponse({ status: 400, description: 'Запись времени не может быть возобновлена' })
  @ApiResponse({ status: 403, description: 'Недостаточно прав доступа' })
  @ApiResponse({ status: 404, description: 'Запись времени не найдена' })
