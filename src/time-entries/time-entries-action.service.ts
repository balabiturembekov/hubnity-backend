...
@Injectable()
export class TimeEntriesActionService {
  private readonly logger = new Logger(TimeEntriesActionService.name);
  ...
  constructor(
    ...
    private internalService: TimeEntriesInternalService,
  ) {}
   ...
   * Enforces one active timer per user: RUNNING/PAUSED entries auto-stop any existing active timer.
   */
  async sync(
    ...
    const createdForBroadcast: Array<{
      id: string;
      userId: string;
    ...
    try {
      ...
      for (const item of dto.entries) {
        ...
        if (status === "RUNNING" || status === "PAUSED") {
          await this.internalService.ensureNoActiveOverlapOrAutoStop(
            tx,
            item.userId,
            companyId,
          );
      ...
      if (
        ...
        const isIdempotencyConflict = target === "idempotencyKey";
        throw new ConflictException(
          isIdempotencyConflict
            ? "Idempotency key already used. Duplicate sync request detected."
    ...
    for (const e of createdForBroadcast) {
      try {
        this.eventsGateway.broadcastTimeEntryUpdate(
          {
            id: e.id,
            userId: e.userId,
        ...
        this.logger.warn(
          `Sync broadcast failed for entry ${e.id}: ${broadcastErr instanceof Error ? broadcastErr.message : "Unknown"}`,
        );
      }
    }
  ...
  async remove(
    ...
    const deleted = await this.prisma.$transaction(async (tx) => {
      ...
      // Never block DB deletion on S3 failure — log for admin cleanup.
      try {
        ...
        this.logger.error(
          ...
          "S3/local file delete failed before TimeEntry removal — DB deletion will proceed. Admin may need to clean up orphaned files manually.",
        );
      }
