...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('should throw ConflictException if company domain already exists', async () => {
      ...
      mockPrismaService.company.findUnique.mockResolvedValue({ id: 'existing-company' });

      await expect(service.register(dtoWithDomain)).rejects.toThrow(ConflictException);
    });

    it('should throw BadRequestException if passwords do not match', async () => {
      const dtoWithMismatch = { ...registerDto, confirmPassword: 'different-password' };

      await expect(service.register(dtoWithMismatch)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if password is too short', async () => {
      const dtoWithShortPassword = { ...registerDto, password: 'short', confirmPassword: 'short' };

      await expect(service.register(dtoWithShortPassword)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if password does not contain letters and numbers', async () => {
      const dtoWithWeakPassword = { ...registerDto, password: 'onlyletters', confirmPassword: 'onlyletters' };

      await expect(service.register(dtoWithWeakPassword)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if email format is invalid', async () => {
      ...
      await expect(service.register(dtoWithInvalidEmail)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should normalize email to lowercase', async () => {
      ...
      const dtoWithUpperCaseEmail = { ...registerDto, email: 'JOHN@EXAMPLE.COM' };
  ...
  describe('login', () => {
    ...
    it('should throw BadRequestException if password is too long', async () => {
      ...
      await expect(service.login(dtoWithLongPassword)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('changePassword', () => {
    ...
    it('should successfully change password', async () => {
      ...
      expect(bcrypt.compare).toHaveBeenNthCalledWith(1, changePasswordDto.currentPassword, mockUser.password);
      expect(bcrypt.compare).toHaveBeenNthCalledWith(2, changePasswordDto.newPassword, mockUser.password);
    ...
    it('should throw BadRequestException if passwords do not match', async () => {
      const dtoWithMismatch = { ...changePasswordDto, confirmPassword: 'different-password' };

      await expect(service.changePassword(userId, dtoWithMismatch)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('resetPassword', () => {
    ...
    it('should throw UnauthorizedException if token is expired', async () => {
      const expiredToken = { ...mockResetToken, expiresAt: new Date(Date.now() - 1000) };
      mockPrismaService.$transaction.mockImplementation(async (callback) => {
        return callback({
          ...mockPrismaService,
          passwordResetToken: {
