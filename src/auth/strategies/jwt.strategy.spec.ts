...
describe('JwtStrategy', () => {
  ...
  describe('validate', () => {
    ...
    it('should throw UnauthorizedException if user ID format is invalid', async () => {
      const invalidPayload = { ...validPayload, sub: 'invalid-uuid-format-not-valid' };
    ...
    it('should throw UnauthorizedException if token is expired', async () => {
      ...
      await expect(strategy.validate(expiredPayload)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should throw UnauthorizedException if companyId is missing', async () => {
      ...
      await expect(strategy.validate(payloadWithoutCompany as any)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should throw UnauthorizedException if companyId mismatch', async () => {
      const payloadWithDifferentCompany = { ...validPayload, companyId: 'different-company-id' };
      mockPrismaService.user.findUnique.mockResolvedValue(mockUser);

      await expect(strategy.validate(payloadWithDifferentCompany)).rejects.toThrow(UnauthorizedException);
    });
  });
});
