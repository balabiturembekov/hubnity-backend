...
import { TeamActivityQueryDto, ActivityPeriod } from './dto/team-activity-query.dto';
...
export class TeamActivityService {
  ...
  getDateRange(period: ActivityPeriod, startDate?: string, endDate?: string): { start: Date; end: Date } {
    const now = new Date();
    let start: Date;
    let end: Date;
    ...
    if (period === ActivityPeriod.CUSTOM && startDate && endDate) {
      ...
      if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        this.logger.warn({ startDate, endDate }, 'Invalid date format provided, falling back to default period');
        ...
        if (start >= end) {
          this.logger.warn({ startDate, endDate }, 'Start date must be before end date, falling back to default period');
          period = ActivityPeriod.LAST_30_DAYS;
        } else {
          return { start, end };
        }
    ...
    if (isNaN(start.getTime()) || isNaN(end.getTime()) || start >= end) {
      this.logger.error({ period, start, end }, 'Invalid date range calculated, using safe fallback');
      start = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
      end = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
    }
  ...
  async getTeamActivity(companyId: string, userId: string, userRole: string, query: TeamActivityQueryDto) {
    const { start, end } = this.getDateRange(query.period || ActivityPeriod.LAST_30_DAYS, query.startDate, query.endDate);
    ...
    if (query.projectId) {
      ...
      if (!projectCheck) {
        throw new NotFoundException(`Project with ID ${query.projectId} not found in your company`);
      }
    ...
    const projectMap = new Map<string, Map<string, { hours: number; earned: number }>>();
    ...
    timeEntries.forEach((entry) => {
      ...
      if (!isFinite(duration) || duration < 0 || duration > 2147483647) {
        this.logger.warn({ entryId: entry.id, duration, userId: entry.userId }, 'Invalid duration value, skipping entry');
        return;
      }

      const hours = duration / 3600;
      if (!isFinite(hours) || hours < 0) {
        this.logger.warn({ entryId: entry.id, hours }, 'Invalid hours calculation, skipping entry');
        return;
      }

      const hourlyRate = member.hourlyRate ?? 0;
      if (!isFinite(hourlyRate) || hourlyRate < 0) {
        this.logger.warn({ userId: member.userId, hourlyRate }, 'Invalid hourlyRate, using 0');
      }

      const earned = hours * hourlyRate;
      if (!isFinite(earned) || earned < 0) {
        this.logger.warn({ entryId: entry.id, earned }, 'Invalid earned calculation, skipping entry');
        return;
      }
      ...
      if (entry.startTime && (!member.lastActivity || entry.startTime > member.lastActivity)) {
        try {
          ...
          this.logger.warn({ entryId: entry.id, startTime: entry.startTime }, 'Invalid startTime format');
        }
      }
    ...
    membersMap.forEach((member, userId) => {
      ...
      member.projectBreakdown = Array.from(userProjectMap.entries())
        .map(([projectId, stats]) => {
          if (!isFinite(stats.hours) || stats.hours < 0 || !isFinite(stats.earned) || stats.earned < 0) {
            this.logger.warn({ userId, projectId, stats }, 'Invalid project stats, skipping');
            return null;
          }

          const entry = timeEntries.find((e) => (e.projectId || 'no-project') === projectId && e.userId === userId);
          return {
    ...
    const members = Array.from(membersMap.values()).sort((a, b) => b.totalHours - a.totalHours);

    const totalHours = members.reduce((sum, m) => {
      const hours = isFinite(m.totalHours) && m.totalHours >= 0 ? m.totalHours : 0;
      return sum + hours;
    }, 0);

    const totalEarned = members.reduce((sum, m) => {
      const earned = isFinite(m.totalEarned) && m.totalEarned >= 0 ? m.totalEarned : 0;
      return sum + earned;
    }, 0);
